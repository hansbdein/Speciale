...
c_oph Correction for handling underflow exception
      DOUBLE PRECISION DAPOS,DXI
...
C
   20 M = MOD(N,4)
      IF (M .EQ. 0) GO TO 40
c_oph Corrected for handling underflow exception
      DAPOS=DABS(DA)
      IF (DLOG10(DAPOS).LT.-290.) DA=DSIGN(1.D-200,DA)
      DO 30 I = 1,M
        DY(I) = DY(I) + DA*DX(I)
   30 CONTINUE
      IF (N .LT. 4) RETURN
...
c_oph Corrected for handling underflow exception
      ...
      DO I=MP1,N
        DXI=DABS(DX(I))
        IF (DXI.NE.0.D0) THEN
          IF (DLOG10(DAPOS)+DLOG10(DXI).GT.-300.) THEN
            DY(I) = DY(I) + DA*DX(I)
          ELSE
            DY(I) = DY(I) + DSIGN(1.D-300,DX(I))
          ENDIF          
        ENDIF
      ENDDO
      RETURN
...
C
   ...
   70 CONTINUE
      RETURN
      END
c_oph DNRM2 in ODEPACK (with obsolescent ASSIGN statements) substituted with DNRM2 from Netlib
