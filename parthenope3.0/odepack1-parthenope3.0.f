...
c! 200  continue
c      write(*,*) 'savf=',savf(1:27)
c      write(*,*) 'ewt=',ewt(1:27)
c      FAC = DVNORM (N, SAVF, EWT)
...
c_oph Corrected for handling underflow exception
          IF ((DMAX1(DABS(YH1(I)),DABS(YH1(I+NYH)))).LT.1.D-30)
     .THEN
            YH1(I) = DSIGN(1.D-30,YH1(I))
          ELSE
...
c_oph Correction for handling underflow exception
        IF (V(I).NE.0..AND.W(I).NE.0.) THEN
          IF ((DABS(V(I)*W(I))).GT.1.D-30)
     .      SUM = SUM + (V(I)*W(I))**2
        ENDIF
 10   CONTINUE
      DVNORM = SQRT(SUM/N)
